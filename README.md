...
## How it Works (Video)
To understand what Huaxiaozhu does and how it works watch this video.
...
## Why Huaxiaozhu was built
Huaxiaozhu is a tool we built so that we (at craft.do) can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or Service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
Huaxiaozhu is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building Huaxiaozhu with Huaxiaozhu only - no code editors - so really, any customisation is just a prompt away.

We built Huaxiaozhu because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
[Check out how I just connected to Slack →](https://docs-aiadp.hxsyai.com/s/DRNQEiy8w2e1v5LPgKl8b)
...
Huaxiaozhu connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
[Here I imported all my skills in one go →](https://docs-aiadp.hxsyai.com/s/gWCFqwhObFWaNJIEJmd6j)
...
**macOS / Linux:**
```bash
curl -fsSL https://docs-aiadp.hxsyai.com/install-app.sh | bash
```

**Windows (PowerShell):**
```powershell
irm https://docs-aiadp.hxsyai.com/install-app.ps1 | iex
```
...
Huaxiaozhu can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
A terminal client that connects to a running Huaxiaozhu server over WebSocket (`ws://` or `wss://`). Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
4. Name: e.g., "Huaxiaozhu Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in Huaxiaozhu
...
Huaxiaozhu supports multiple ways to connect to LLM providers:
...
Huaxiaozhu uses two agent backends:
...
See the [Automations documentation](https://docs-aiadp.hxsyai.com/docs/automations/overview) for the full reference.
...
External apps can navigate using `huaxiaozhu://` URLs:

```
huaxiaozhu://allSessions                      # All sessions view
huaxiaozhu://allSessions/session/session123   # Specific session
huaxiaozhu://settings                         # Settings
huaxiaozhu://sources/source/github            # Source info
huaxiaozhu://action/new-chat                  # Create new session
```
...
**Windows (PowerShell):**
```powershell
& "$env:LOCALAPPDATA\Programs\@craft-agentelectron\Huaxiaozhu.exe" -- --debug
```
...
Huaxiaozhu（华小竹）is this distribution's product name. Original upstream trademarks remain owned by their respective owner; see [TRADEMARK.md](TRADEMARK.md).
