...
export const UpdatePreferencesSchema = z.object({
  ...
  includeCoAuthoredBy: z.boolean().optional().describe("Whether to include 'Co-Authored-By: Huaxiaozhu' trailer on git commits. Defaults to true."),
});
...
- Do NOT include any text or tool calls after SubmitPlan - they will not be executed`,
  ...
  config_validate: `Validate Huaxiaozhu configuration files.
...
Only use 'attachments' for existing file paths on disk — the tool reads them automatically.`,
  ...
  send_developer_feedback: `Send freeform feedback to the Huaxiaozhu development team.
...
Requires an explicit sessionId and cannot target your own session. Use list_sessions / get_session_info to find the target session's ID.`,
  ...
  create_task: `Create a Huaxiaozhu Task on the kanban board — writes tasks/<slug>/task.yaml and creates its orchestrator session. CREATION ONLY: the task lands in "todo" and is NOT run; starting it is the user's (or an automation's) decision.
