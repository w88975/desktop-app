...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = options.callbackUrl
    ?? `https://api-aidp.hxsyai.com/v1/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects: https://api-aidp.hxsyai.com/v1/auth/slack/callback → http://localhost:{port}/callback
    const redirectUri = `https://api-aidp.hxsyai.com/v1/auth/slack/callback?port=${port}`;
