...
describe('registerSystemCoreHandlers OPEN_URL', () => {
  it('routes huaxiaozhu action links internally via deeplink:navigate', async () => {
    const { openUrl, ctx, invokeClientCalls, pushCalls } = createTestHarness()

    await openUrl(ctx, 'huaxiaozhu://action/new-session?input=sg&send=true')
  ...
  it('routes workspace deep links to workspace target when URL workspace differs', async () => {
    ...
    await openUrl(ctx, 'huaxiaozhu://workspace/ws-2/action/new-session?input=hello')
  ...
  it('falls back to client openExternal for huaxiaozhu window-mode links', async () => {
    const { openUrl, ctx, invokeClientCalls, pushCalls } = createTestHarness()

    await openUrl(ctx, 'huaxiaozhu://action/new-session?window=focused')
    ...
    expect(invokeClientCalls[0]).toEqual({
      ...
      args: ['huaxiaozhu://action/new-session?window=focused'],
    })
  })
