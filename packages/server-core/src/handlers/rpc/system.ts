...
function parseInternalHuaxiaozhuDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'huaxiaozhu:') return null
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Shell operations - open URL in external browser (or handle huaxiaozhu:// internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalHuaxiaozhuDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing huaxiaozhu:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for huaxiaozhu:// URL')
