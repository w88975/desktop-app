...
深链（`craftagents://`）由主进程的 `deep-link.ts` 解析后转成同一套路由。
...
级联主题：应用级 + workspace 级。workspace 的 `defaults.colorTheme` 覆盖应用默认。

- 主题定义 `~/.craft-agent/themes/`
- 色彩类型 `packages/shared/src/colors/`
- 渲染侧 `renderer/context/ThemeContext.tsx` + `hooks/useTheme.ts`
- Tailwind v4 + CSS variables，全局入口 `renderer/index.css`
