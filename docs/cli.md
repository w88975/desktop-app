...
Terminal client for Huaxiaozhu server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running Huaxiaozhu headless server with URL and token
