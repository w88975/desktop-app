...
export function AppShell(props: AppShellProps) {
  ...
  useEffect(() => {
    ...
    if (!sessionId || lastSyncedSessionIdRef.current === sessionId) return
    lastSyncedSessionIdRef.current = sessionId
    const syncAgentConversation = window.electronAPI.syncAgentConversation
    if (typeof syncAgentConversation === 'function') {
      void syncAgentConversation(sessionId).catch(error => {
        console.warn('[AppShell] Failed to synchronize Agent conversation', error)
      })
    }
  }, [session.selected])
