...
const api: ShellAPI = {
  ...
  reorderTab: (tabId, targetTabId, placement) => ipcRenderer.invoke(
    APP_PLATFORM_CHANNELS.REORDER_TAB,
    tabId,
    targetTabId,
    placement,
  ),
  setTabOrder: (tabIds) => ipcRenderer.invoke(APP_PLATFORM_CHANNELS.SET_TAB_ORDER, tabIds),
  closeTabsRight: (tabId) => ipcRenderer.invoke(APP_PLATFORM_CHANNELS.CLOSE_TABS_RIGHT, tabId),
  closeOtherTabs: (tabId) => ipcRenderer.invoke(APP_PLATFORM_CHANNELS.CLOSE_OTHER_TABS, tabId),
