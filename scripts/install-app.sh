...
VERSIONS_URL="https://docs-aiadp.hxsyai.com/electron"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Huaxiaozhu-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.huaxisy.huaxiaozhu"
    if pgrep -x "华小竹" >/dev/null 2>&1; then
        info "Quitting Huaxiaozhu..."
    ...
    echo ""
    printf "%b\n" "  Huaxiaozhu has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    ...
    WRAPPER_PATH="$INSTALL_DIR/huaxiaozhu"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Huaxiaozhu-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Craft-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping Huaxiaozhu..."
...
#!/bin/bash
# Huaxiaozhu launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.craft-agent/app/Huaxiaozhu-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@craft-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@craft-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Huaxiaozhu not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://docs-aiadp.hxsyai.com/install-app.sh | bash"
    exit 1
fi
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Huaxiaozhu-x64.AppImage"
