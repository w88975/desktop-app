# Huaxiaozhu Windows Installer
# Usage: irm https://docs-aiadp.hxsyai.com/install-app.ps1 | iex
...
$process = Get-Process -Name "华小竹" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing 华小竹..."
...
Write-Info "Adding 'huaxiaozhu' command to PATH..."

$binDir = "$env:LOCALAPPDATA\华小竹\bin"
$cmdFile = "$binDir\huaxiaozhu.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\华小竹\华小竹.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'huaxiaozhu' command)"
} else {
    Write-Success "Command 'huaxiaozhu' is ready"
}
...
Write-Host ""
Write-Host "  Huaxiaozhu has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: huaxiaozhu (restart terminal first)"
Write-Host ""
}
