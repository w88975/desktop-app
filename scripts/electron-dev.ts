...
function detectInstance(): void {
  ...
  if (match) {
    ...
    process.env.CRAFT_APP_NAME = `华小竹 [${instanceNum}]`;
    process.env.CRAFT_CONFIG_DIR = join(process.env.HOME || "", `.craft-agent-${instanceNum}`);
    process.env.CRAFT_DEEPLINK_SCHEME = `huaxiaozhu${instanceNum}`;
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || "华小竹",
    CRAFT_DEEPLINK_SCHEME: process.env.CRAFT_DEEPLINK_SCHEME || "huaxiaozhu",
...
async function main(): Promise<void> {
  ...
  const authPreloadCjsPath = join(DIST_DIR, "auth-preload.cjs");
  const settingsPreloadCjsPath = join(DIST_DIR, "settings-preload.cjs");
  ...
  if (existsSync(authPreloadCjsPath)) rmSync(authPreloadCjsPath);
  if (existsSync(settingsPreloadCjsPath)) rmSync(settingsPreloadCjsPath);
  ...
  const [mainResult, preloadResult, toolbarPreloadResult, shellPreloadResult, authPreloadResult, settingsPreloadResult, appHostPreloadResult, externalAppPreloadResult] = await Promise.all([
    runEsbuild(
    ...
    runEsbuild(
      "apps/electron/src/preload/auth.ts",
      "apps/electron/dist/auth-preload.cjs"
    ),
    runEsbuild(
      "apps/electron/src/preload/settings.ts",
      "apps/electron/dist/settings-preload.cjs"
    ),
    runEsbuild(
  ...
  if (!shellPreloadResult.success || !authPreloadResult.success || !settingsPreloadResult.success || !appHostPreloadResult.success || !externalAppPreloadResult.success) {
    console.error("❌ App-platform preload build failed:", shellPreloadResult.error || authPreloadResult.error || settingsPreloadResult.error || appHostPreloadResult.error || externalAppPreloadResult.error);
    process.exit(1);
  }

  // Wait for files to stabilize (filesystem flush)
  console.log("⏳ Waiting for build files to stabilize...");
  const [mainStable, preloadStable, toolbarPreloadStable, shellPreloadStable, authPreloadStable, settingsPreloadStable, appHostPreloadStable, externalAppPreloadStable] = await Promise.all([
    ...
    waitForFileStable(authPreloadCjsPath),
    waitForFileStable(settingsPreloadCjsPath),
    waitForFileStable(appHostPreloadCjsPath),
    waitForFileStable(externalAppPreloadCjsPath),
  ]);

  if (!mainStable || !preloadStable || !toolbarPreloadStable || !shellPreloadStable || !authPreloadStable || !settingsPreloadStable || !appHostPreloadStable || !externalAppPreloadStable) {
  ...
  const [mainValid, preloadValid, toolbarPreloadValid, shellPreloadValid, authPreloadValid, settingsPreloadValid, appHostPreloadValid, externalAppPreloadValid] = await Promise.all([
    verifyJsFile(mainCjsPath),
    verifyJsFile(preloadCjsPath),
    verifyJsFile(toolbarPreloadCjsPath),
    verifyJsFile(shellPreloadCjsPath),
    verifyJsFile(authPreloadCjsPath),
    verifyJsFile(settingsPreloadCjsPath),
  ...
  if (!shellPreloadValid.valid || !authPreloadValid.valid || !settingsPreloadValid.valid || !appHostPreloadValid.valid || !externalAppPreloadValid.valid) {
    console.error("❌ App-platform preload is invalid:", shellPreloadValid.error || authPreloadValid.error || settingsPreloadValid.error || appHostPreloadValid.error || externalAppPreloadValid.error);
    process.exit(1);
  }
  ...
  for (const [entry, outfile, label] of [
    ...
    ["apps/electron/src/preload/auth.ts", "apps/electron/dist/auth-preload.cjs", "auth preload"],
    ["apps/electron/src/preload/settings.ts", "apps/electron/dist/settings-preload.cjs", "settings preload"],
